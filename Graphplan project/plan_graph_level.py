            ...
            if previous_proposition_layer.all_preconds_in_layer(act):
                ...
                for i in range(len(prop)):
                    for j in range(len(prop)):
                        if previous_proposition_layer.is_mutex(prop[i], prop[j]):
                            flag = False
                if flag:
    ...
    def update_proposition_layer(self):
        ...
        for action in current_layer_actions:
            for proposition in action.get_add():
                if proposition in self.proposition_layer.get_propositions():
                    continue
                self.proposition_layer.add_proposition(proposition)
                if action not in proposition_dict.keys():
                    proposition_dict[action] = proposition
...
def mutex_propositions(prop1, prop2, mutex_actions_list):
    ...
    """
    for act_1 in prop1.get_producers():
        for act_2 in prop2.get_producers():
            if Pair(act_1, act_2) not in mutex_actions_list:
                return False
