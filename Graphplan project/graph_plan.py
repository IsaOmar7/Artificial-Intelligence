...
def independent_pair(a1, a2):
    ...
    """
    for act in a1.get_delete():
        # is_pre_con checks if actions interfere with each other.
        # is_pos effect checks if actions have inconsistent effects.
        if a2.is_pre_cond(act) or a2.is_pos_effect(act):
            return False
    for act in a2.get_delete():
        if a1.is_pre_cond(act) or a1.is_pos_effect(act):
            return False
    return True
