...
class PlanningProblem:
    ...
    def get_successors(self, state):
        ...
        for action in valid_actions:
            del_propositions = state.difference(action.get_delete())
            full_proposition = del_propositions.union(action.get_add())
            final_proposition = frozenset(full_proposition)
            succ_lst.append((final_proposition, action, 1))
        return succ_lst
...
def level_sum(state, planning_problem):
    ...
    for prop in state:
        if prop in planning_problem.goal:
            if prop not in sub_goals:
                sub_goals.append(prop)
    fixed_graph = []
    fixed_graph.append(pg_init)
    while not planning_problem.is_goal_state(pg_init.proposition_layer.propositions):
        if is_fixed(fixed_graph, number_of_levels):
        ...
        pg_init.update_proposition_layer()
        fixed_graph.append(pg_init)
        for prop in pg_init.proposition_layer.propositions:
            if prop not in sub_goals:
                sub_goals.append(prop)
                number_of_subgoals_level += number_of_levels

    return number_of_subgoals_level
